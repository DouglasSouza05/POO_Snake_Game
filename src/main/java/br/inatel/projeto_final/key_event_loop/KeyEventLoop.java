package br.inatel.projeto_final.key_event_loop;
...
public class KeyEventLoop {
    ...
    public KeyEventLoop(Scenario scenario, Snake snake, Apple apple, Pineapple pineapple) {
        ...
        //Usando sintaxe lâmbida para que ao acontecer tal evento "e", realizar certas ações
        this.scenario.keyPressed(e -> {

            KeyCode key = e.getCode(); //verifica qual tecla foi pressionada

            //!(currentDirection.equals(KeyCode.LEFT)) impede que a cobra vá na direção contrária
            //a que estava indo
            //Direita
    ...
    public void startLoop() {
        ...
        keyFrame = new KeyFrame(Duration.millis(160), e -> {
            ...
            //-Config.squareSize é usado pois quando posicionX = Config.width uma parte ja está fora da tela
            //Tambem faz a verificação se não houve colisão entre head e algum elemento de body
            ...
            || snake.bodyCollision(posicionX, posicionY)) {
                ...
                //Verifica se a posição da head de snake está na mesma posição de food (apple ou pineapple)
